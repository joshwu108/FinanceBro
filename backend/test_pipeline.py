...
async def test_single_stock():
    """Test the pipeline with a single stock"""
    print("🧪 Testing FinanceBro ML Pipeline")
    print("=" * 40)
    ...
    symbol = "AAPL"
    period = "3mo"  # 3 months for quick testing
    ...
    X_train, X_test, y_train, y_test = feature_engineer.prepare_ml_data(
        features_df, target_column='target_direction_1d', test_size=0.2
    )
    ...
    print("💡 You can now run the full pipeline with multiple stocks!")
