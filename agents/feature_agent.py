...
CRITICAL fixes applied vs. the original service:
  - Look-ahead bias in target construction (shift(-days) misuse) — FIXED
  - NaN handling changed from ffill/bfill to dropna — FIXED
  - Raw OHLCV excluded from feature columns — FIXED
  - Feature/target strictly separated — ENFORCED
  - days_since_start removed (leaks dataset boundary) — REMOVED
  - feature_columns accumulation bug — FIXED (reset each run)
  - Rolling std now computed on returns, not price levels — FIXED
"""
